...
**Technical**: LLM context limits (memory compression needed), capability variance (graceful degradation), research accuracy (player verification), state persistence (JSON export fragility)

**Design**: Single-player only (no true multiplayer), narrative > balance (approximate mechanics), genre focus (popular anime covered, niche improvised), high-quality LLM dependency
